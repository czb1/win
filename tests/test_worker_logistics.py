...
class LogisticsReplayTests(unittest.TestCase):
    ...
    def test_rebuild_and_expand_for_three_days_without_stranding_operators(self):
        for mirror in (False, True):
            with self.subTest(mirror=mirror):
                result = simulate(Agent, Config, mirror=mirror, days=3, damage_walls=True)
                self.assertEqual(result["invalid_actions"], 0)
                ...
                # Core upgrades may defer untouched side walls on day two;
                # carried stone is still spent and expansion resumes later.
                self.assertGreaterEqual(nights[1]["walls"], nights[0]["walls"])
                self.assertGreaterEqual(nights[2]["walls"], nights[0]["walls"] + 3)
