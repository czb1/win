    ...
    def test_remote_ore_keeps_all_operators_and_front_wall_ready(self):
        for mirror in (False, True):
            with self.subTest(mirror=mirror):
                ...
                self.assertEqual(result["checkpoints"]["69"]["operators_used"], 1)
    ...
    def test_rebuild_front_for_three_days_without_stranding_operators(self):
        for mirror in (False, True):
            with self.subTest(mirror=mirror):
                ...
                self.assertTrue(all(n["operators_ready"] == 3 and n["operators_used"] == 1
                                    and n["front_walls"] == 6 for n in nights))
