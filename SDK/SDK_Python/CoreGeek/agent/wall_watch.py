...
def select_watch(turn, mem, pairs):
    ...
    for wall in (w for w in turn.ours if w.kind == 'wall'):
        key = (wall.pos, wall.level)
        mem.wall_watch_health[key] = max(1000, wall.health, mem.wall_watch_health.get(key, 0))
...
def repair_watch(turn, mem, nav, ledger, hero, sites):
    if not hero.inventory['WallFixer']:
        return False
    ...
    for wall in turn.ours:
        if (wall.kind != 'wall' or wall.pos not in sites or wall.id in ledger.repair_claims
                or wall.health >= mem.wall_watch_health.get((wall.pos, wall.level), 1000)):
            continue
        route = watch_route(turn, nav, ledger, mem, hero, sites, wall)
        if route:
            peak = mem.wall_watch_health.get((wall.pos, wall.level), 1000)
            options.append((wall.pos[0] != front, wall.health / peak, route[0], wall.id, wall, route))
    if not options:
        return False
