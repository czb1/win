...
最新工人策略：保持采矿、集中出售与建设的阶段安排；记录移动失败和重复绕圈，运输途中保持目标；建墙石料默认每批 10 块，并保留施工库存。取消炮台未满级时只能建迎敌面六格墙的限制，持续扩建与补洞。回防同时分配炮台和独立站位，在狭窄通道处等待或让行。策略由程序执行，不依赖任务模型的推理能力。改动、对照结果与复现方法见 [工人运输、视野与持续建墙](docs/worker-logistics.md)。

此前持续采矿、批量购物和第 70 回合备战改进见 [持续采矿与第 70 回合备战](docs/steady-mining.md)。
...
同步 `main` 的 `0bd97af` 后，Python 3.11 虚拟环境下全部 188 项测试通过。相同八组首日受控回放中，一组附近铁矿场景采集由 53 次增至 56 次（增加 3 块未售石头），其余七组夜前状态一致；所有场景均保留主分支的首夜火力、围墙和三名操控者到位，非法动作均为 0。详情及限制见 [主分支整合验证](docs/personal-hjc-main-integration.md)。
