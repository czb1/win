...
最新工人策略：默认只建迎敌面六格城墙，缺口就近优先补齐；夜间附近没有可见威胁时立即恢复采矿，威胁接近则提前回防。每名工人每个白天最多集中出售一批资源，多种矿在同一次到店过程中逐项卖完；卖后新采矿留到次日。采矿、卖矿、建墙、回防均由确定性代码执行，不依赖弱模型判断。具体规则与验证见 [迎敌面、夜间采矿与每日一次卖矿](docs/front-night-sales.md)。

此前移动纠错、运输目标保持与协同回防见 [工人运输、视野与持续建墙](docs/worker-logistics.md)。
...
默认左侧基地只建右侧迎敌墙，右侧基地只建左侧迎敌墙，常规位置共六格；侧后方保持开放，供采矿、购物和回防使用。显式 `wall_cells` 布局仍按配置执行。自定义直射武器请配套显式布局，否则连续墙会挡住其弹道。
