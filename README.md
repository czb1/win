...
最新经济策略：持续采完当前矿区，按实际路程、已观察余量和两名工人的分工选矿，减少小批量卖矿与折返；采购员完成整次买券和交付，另一人顺路备石料。默认第 70 回合进入首夜，第 69 回合结束前完成首轮建设、升级与操控者回防，开拓者的未完成任务也让位于第一次备战。经济与防御不依赖模型判断；附近漏矿、首夜就绪和多日对比见 [持续采矿与第 70 回合备战](docs/steady-mining.md)。

当前补充修复：不可达炮台不再占用工人，全部炮台不可达时可改以基地规划采矿返程。正常采矿、出售和建设无法继续时，工人可尝试实际步行两步以内的矿点；按实际采集站位核对回防路程，保留默认 5 回合余量，矿石允许留到次日出售。无法在当天完成的卖矿往返不会再强制发起。该补充不会取消原有备战截止时间，也不会主动远行囤矿。

这次修复在 Python 3.11 虚拟环境下通过全部 162 项测试。相同首日受控回放中，近商店采集保持 42 次，远商店由 26 次增至 27 次（多一块未售石头）；两个场景均保持原有首夜火力、迎敌围墙和三名操控者到位，非法动作均为 0。回放不包含官方对战结算，不能据此推断实战胜率。
...
使用旧配置时请同步更新 `round_origin: 0`、`sell_batch: 40`、`sell_batch_max: 80`，否则旧配置会覆盖新默认值。其他默认策略：默认 `loadout` 现为三座 `rocket`，`task_max_rounds` 改为 1300（整局上限）。实际任务仍受官方 `timeoutRounds` 和回防时间约束；手动保留的 40 回合配置仍会提前限制长任务。
